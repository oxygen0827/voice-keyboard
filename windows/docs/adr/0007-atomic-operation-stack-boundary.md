...
An Atomic Operation Stack should contain two operations by default and at most three when every operation is explicit and low risk. The engine executes the stack in order. If one operation fails, later operations do not run. The engine does not auto-repair or auto-rollback a stack.

Dictation Mode does not participate in stacks, although Instruction Mode may include a Text Insertion operation when the text is explicit or resolved from Memo. Spoken undo is a Shortcut Invocation of the current Input Environment undo action.
