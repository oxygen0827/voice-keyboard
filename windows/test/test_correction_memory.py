...
class CorrectionMemoryTests(unittest.TestCase):
    ...
    def test_batch_edit_counts_each_changed_occurrence(self):
        with tempfile.TemporaryDirectory() as tmp:
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=3)

            result = memory.record_observation(
                "王之行，王之行，王之行",
                "王知行，王知行，王知行",
            )
            ...
            self.assertEqual(
                [(item.wrong, item.correct, item.count, item.newly_confirmed) for item in result.confirmed],
                [("王之行", "王知行", 3, True)],
            )
            self.assertEqual(memory.apply("王之行来了"), "王知行来了")

    def test_single_changed_occurrence_in_repeated_segment_counts_once(self):
        with tempfile.TemporaryDirectory() as tmp:
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=3)
            ...
            self.assertEqual(
                [(item.wrong, item.correct, item.count, item.newly_confirmed) for item in result.candidates],
                [("王之行", "王知行", 1, False)],
            )
            self.assertEqual(memory.apply("王之行来了"), "王之行来了")

    def test_infers_two_to_five_character_terms(self):
        cases = [
            ("王琦，王琦", "王齐，王齐", "王琦", "王齐"),
            ("胡人远，胡人远", "胡任远，胡任远", "胡人远", "胡任远"),
            ("欧阳志行，欧阳志行", "欧阳知行，欧阳知行", "欧阳志行", "欧阳知行"),
            ("司徒王知行，司徒王知行", "司徒王之行，司徒王之行", "司徒王知行", "司徒王之行"),
        ]

        for before, after, wrong, correct in cases:
            with self.subTest(wrong=wrong):
                corrections = infer_correction_pairs(before, after)
                self.assertIn(
                    (wrong, correct),
                    [(item.wrong, item.correct) for item in corrections],
                )
    ...
    def test_tracker_observes_pending_dictation_against_current_text(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(result.candidates, ())
            self.assertEqual(
                [(item.wrong, item.correct, item.count) for item in result.confirmed],
                [("王琦", "王齐", 3)],
            )
    ...
    def test_tracker_keeps_earlier_pending_dictation_when_another_utterance_arrives(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [(item.wrong, item.correct, item.count) for item in result.candidates],
                [("王琪", "王齐", 1)],
            )
    ...
    def test_tracker_single_repeated_name_edit_records_candidate_without_confirming(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [(item.wrong, item.correct, item.count) for item in first.candidates],
                [("文静", "文净", 1)],
            )
            self.assertEqual(memory.apply("文静"), "文静")

    def test_single_edit_of_repeated_spoken_name_records_candidate_from_full_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp:
            current = {"text": "文静，文静，文净"}
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=2)
            tracker = CorrectionLearningTracker(
                memory,
                lambda: CorrectionTextSnapshot(current["text"], source="Win32:WM_GETTEXT"),
            )
            ...
            self.assertEqual(memory.apply("文静，文静，文静"), "文静，文静，文静")
    ...
    def test_tracker_does_not_count_same_candidate_observation_twice(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(memory.apply("文静"), "文静")
            self.assertEqual(memory.candidates[0].count, 1)
    ...
    def test_tracker_from_config_disables_screen_ocr_by_default(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            tracker = CorrectionLearningTracker.from_config(
                ...
                read_clipboard_text=lambda reference: (
                    clipboard_calls.append(reference)
                    or CorrectionTextSnapshot("", source="clipboard_probe")
                ),
            )
    ...
    def test_tracker_from_config_can_enable_clipboard_probe_explicitly(self):
        with tempfile.TemporaryDirectory() as tmp:
            now = {"value": 100.0}
            clipboard_calls = []
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=1)
            tracker = CorrectionLearningTracker.from_config(
                {"clipboard_probe_fallback": True},
                memory,
                lambda: CorrectionTextSnapshot("", source="unsupported", detail="AXWindowScan:no"),
                read_clipboard_text=lambda reference: (
                    clipboard_calls.append(reference)
                    or CorrectionTextSnapshot("李立夫", source="clipboard_probe")
                ),
            )
            ...
            self.assertEqual(clipboard_calls, ["李丽夫"])
            self.assertEqual(
                [(item.wrong, item.correct) for item in result.confirmed],
                [("李丽夫", "李立夫")],
            )
    ...
    def test_tracker_learns_from_repeated_committed_replacements_without_text_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [(item.wrong, item.correct, item.count) for item in second.confirmed],
                [("文静", "文净", 2)],
            )
    ...
    def test_tracker_uses_clipboard_probe_when_chrome_text_capture_is_unavailable(self):
        with tempfile.TemporaryDirectory() as tmp:
            now = {"value": 100.0}
            clipboard_calls = []
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=2)
            tracker = CorrectionLearningTracker.from_config(
                {"debug": True, "clipboard_probe_fallback": True},
                memory,
                lambda: CorrectionTextSnapshot(
                    "胡任远，王之行，王之行王之行",
                    source="tracked_segment",
                    detail="fallback_after=unsupported;role=Chrome_WidgetWin_1",
                ),
                read_clipboard_text=lambda reference: (
                    clipboard_calls.append(reference)
                    or CorrectionTextSnapshot(
            ...
            tracker._clock = lambda: now["value"]
            tracker.remember_inserted("胡任远，王之行，王之行王之行")
            tracker.record_key_press(SimpleNamespace(name="backspace"))
            for char in "zhi ":
                tracker.record_key_press(SimpleNamespace(char=char))
            now["value"] += 1.0
            ...
            self.assertEqual(clipboard_calls, ["胡任远，王之行，王之行王之行"])
            self.assertEqual(result.candidates, ())
            self.assertEqual(
                [(item.wrong, item.correct, item.count) for item in result.confirmed],
                [("王之行", "王知行", 3)],
            )
            self.assertEqual(memory.apply("王之行"), "王知行")
            self.assertEqual(result.decisions[-1].source, "clipboard_probe")
    ...
    def test_tracker_learns_windows_ime_correction_from_focused_text_snapshot(self):
        with tempfile.TemporaryDirectory() as tmp:
            current = {
                "snapshot": CorrectionTextSnapshot(
                    "文静，文静，文静",
                    source="Win32:WM_GETTEXT",
                    detail="confidence=medium",
                )
            }
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=2)
            tracker = CorrectionLearningTracker(memory, lambda: current["snapshot"])
            ...
            for char in "jing":
                tracker.record_key_press(SimpleNamespace(char=char))
            tracker.record_key_press(SimpleNamespace(name="space"))
            current["snapshot"] = CorrectionTextSnapshot(
                "文净，文净，文净",
                source="Win32:WM_GETTEXT",
                detail="confidence=medium",
            )
            ...
            self.assertEqual(result.decisions[-1].source, "Win32:WM_GETTEXT")
    ...
    def test_tracker_counts_repeated_term_when_ime_commit_replaces_one_occurrence(self):
        with tempfile.TemporaryDirectory() as tmp:
            current = {
                "snapshot": CorrectionTextSnapshot(
                    "",
    ...
    def test_tracker_does_not_learn_previous_dictation_from_next_utterance_with_same_prefix(self):
        with tempfile.TemporaryDirectory() as tmp:
            current = {
                "snapshot": CorrectionTextSnapshot(
                    "文好像还是没有出现已添加到词典的提示",
                    source="UIAutomation:ValuePattern",
                )
            }
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=1)
            tracker = CorrectionLearningTracker(memory, lambda: current["snapshot"])
            ...
            tracker.remember_inserted("文静，文静，文静")
            tracker.remember_inserted("好像还是没有出现已添加到词典的提示")
            result = tracker.observe_current_text()
    ...
    def test_tracker_does_not_learn_window_title_after_manual_edit(self):
        with tempfile.TemporaryDirectory() as tmp:
            current = {
                "snapshot": CorrectionTextSnapshot(
                    "微信",
                    source="Win32:WM_GETTEXT",
                    detail="confidence=medium role=Qt51514QWindowIcon",
                )
            }
            memory = CorrectionMemory(Path(tmp) / "correction.json", confirm_threshold=1)
            tracker = CorrectionLearningTracker(memory, lambda: current["snapshot"])
            ...
            tracker.remember_inserted("文静，文静，文静")
            tracker.record_key_press(SimpleNamespace(name="backspace"))
            tracker.record_key_press(SimpleNamespace(char="j"))
            result = tracker.observe_current_text()

            self.assertEqual(result.confirmed, ())
            self.assertEqual(result.candidates, ())
            self.assertEqual(memory.apply("文静"), "文静")
            self.assertEqual(result.decisions[-1].reason, "current_text_not_same_segment")

    def test_tracker_does_not_learn_from_next_tracked_segment_after_manual_edit(self):
        with tempfile.TemporaryDirectory() as tmp:
            current = {
                "snapshot": CorrectionTextSnapshot(
                    "感觉还是没有加到词典里面去的提示",
            ...
            self.assertEqual(result.candidates, ())
            self.assertEqual(memory.apply("文静"), "文静")
            self.assertEqual(result.decisions[-1].reason, "tracked_segment_replaced_pending")
    ...
    def test_scheduler_reports_confirmed_corrections(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            scheduler.stop()
            self.assertEqual(
                [(item.wrong, item.correct, item.count) for item in observed],
                [("王琦", "王齐", 3)],
            )
