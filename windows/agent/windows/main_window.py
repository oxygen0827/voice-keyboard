...
from agent.correction_memory import CorrectionEntry, CorrectionMemory
...
class WindowsMainWindow:
    def __init__(self, *, history: History, insert_text, reload_config, notify):
        ...
        self._correction_memory = self._new_correction_memory()
        ...
        self._dictionary_entries: list[CorrectionEntry] = []
        self._dictionary_candidates: list[CorrectionEntry] = []
        self._checked_dictionary_entries: set[str] = set()
        self._checked_dictionary_candidates: set[tuple[str, str]] = set()
    ...
    def _build_dictionary_tab(self) -> None:
        ...
        ttk.Button(toolbar, text=self._t("\u6253\u5f00\u6587\u4ef6", "Open File"), command=self._open_dictionary_file).pack(side="right")

        body = ttk.PanedWindow(tab, orient="vertical")
        body.pack(fill="both", expand=True, pady=(8, 0))

        added = ttk.Frame(body)
        body.add(added, weight=3)
        ttk.Label(added, text=self._t("\u5df2\u52a0\u5165\u7684\u8bcd\u5178", "Added Dictionary")).pack(anchor="w")
        self._dictionary_entries_tree = self._build_correction_tree(added)
        self._dictionary_entries_tree.bind("<Button-1>", lambda event: self._toggle_dictionary_tree_check(event, "entry"))
        self._dictionary_entries_tree.bind("<<TreeviewSelect>>", lambda _event: self._sync_dictionary_tree_selection("entry"))

        candidates = ttk.Frame(body)
        body.add(candidates, weight=2)
        ttk.Label(candidates, text=self._t("\u5019\u9009\u8bcd\u5178", "Candidate Dictionary")).pack(anchor="w")
        self._dictionary_candidates_tree = self._build_correction_tree(candidates)
        self._dictionary_candidates_tree.bind("<Button-1>", lambda event: self._toggle_dictionary_tree_check(event, "candidate"))
        self._dictionary_candidates_tree.bind("<<TreeviewSelect>>", lambda _event: self._sync_dictionary_tree_selection("candidate"))
    ...
    def _refresh_overview(self) -> None:
        ...
        rows = [
            ...
            f"Correction Dictionary: {self._correction_memory_path()}",
            f"Personal Dictionary: {_USER_DIR / 'personal_dictionary.json'}",
    ...
    def _refresh_memo(self) -> None:
        if not hasattr(self, "_memo_list"):
            return
        self._memo = MemoStore()
    ...
    def _build_correction_tree(self, parent) -> ttk.Treeview:
        cols = ("checked", "wrong", "correct", "count", "updated")
        tree = ttk.Treeview(parent, columns=cols, show="headings", height=9, selectmode="browse")
        headings = {
            "checked": "\u2610",
            "wrong": self._t("\u539f\u8bcd", "Before"),
            "correct": self._t("\u4fee\u6b63\u4e3a", "After"),
            "count": self._t("\u6b21\u6570", "Count"),
            "updated": self._t("\u66f4\u65b0\u65f6\u95f4", "Updated"),
        }
        widths = {
            "checked": 48,
            "wrong": 180,
    ...
    def _refresh_dictionary(self) -> None:
        if not hasattr(self, "_dictionary_entries_tree"):
            return
        self._correction_memory = self._new_correction_memory()
        self._dictionary_entries = sorted(
            self._correction_memory.entries,
            key=lambda entry: entry.updated_at,
            reverse=True,
        )
        self._dictionary_candidates = sorted(
            self._correction_memory.candidates,
            key=lambda entry: entry.updated_at,
            reverse=True,
        )
        entry_keys = {entry.wrong for entry in self._dictionary_entries}
        candidate_keys = {(entry.wrong, entry.correct) for entry in self._dictionary_candidates}
        self._checked_dictionary_entries &= entry_keys
        self._checked_dictionary_candidates &= candidate_keys
        self._populate_correction_tree(
            self._dictionary_entries_tree,
            self._dictionary_entries,
            "entry",
        )
        self._populate_correction_tree(
            self._dictionary_candidates_tree,
            self._dictionary_candidates,
            "candidate",
        )

    def _populate_correction_tree(
        self,
        tree: ttk.Treeview,
        rows: list[CorrectionEntry],
        kind: str,
    ) -> None:
        tree.delete(*tree.get_children())
        checked = (
            self._checked_dictionary_entries
            if kind == "entry"
            else self._checked_dictionary_candidates
        )
        for index, entry in enumerate(rows):
            key = entry.wrong if kind == "entry" else (entry.wrong, entry.correct)
            ...
            tree.insert(
                ...
                values=(
                    ...
                    entry.correct,
                    entry.count,
                    self._format_time(entry.updated_at),
                ),
            )
    ...
    def _toggle_dictionary_tree_check(self, event, kind: str) -> None:
        ...
        if not iid:
            return
        entry = self._dictionary_row(kind, iid)
        if entry is None:
            return
        if kind == "entry":
            self._toggle_set_value(self._checked_dictionary_entries, entry.wrong)
            self._populate_correction_tree(tree, self._dictionary_entries, kind)
        else:
    ...
    def _delete_checked_dictionary_items(self) -> None:
        count = len(self._checked_dictionary_entries) + len(self._checked_dictionary_candidates)
        if count <= 0:
            return
        message = self._t(
            f"\u786e\u5b9a\u5220\u9664\u52fe\u9009\u7684 {count} \u4e2a\u8bcd\u6761\uff1f",
            f"Delete {count} checked dictionary items?",
        )
        if not messagebox.askyesno("Voice Keyboard", message):
            return
        deleted = 0
        self._correction_memory = self._new_correction_memory()
        for wrong in tuple(self._checked_dictionary_entries):
            if self._correction_memory.delete_entry(wrong):
                deleted += 1
        for wrong, correct in tuple(self._checked_dictionary_candidates):
            if self._correction_memory.delete_candidate(wrong, correct):
                deleted += 1
        self._checked_dictionary_entries.clear()
        self._checked_dictionary_candidates.clear()
        self._refresh_dictionary()
        self._notify_text(self._t(f"\u5df2\u5220\u9664 {deleted} \u4e2a\u8bcd\u6761", f"Deleted {deleted} dictionary items"))

    def _copy_selected_dictionary_item(self) -> None:
        entry = self._selected_dictionary_entry()
        if entry is None:
            return
        text = f"{entry.wrong} -> {entry.correct}\ncount: {entry.count}"
        self._copy_text(text)

    def _selected_dictionary_entry(self) -> CorrectionEntry | None:
        selection = self._dictionary_entries_tree.selection()
        if selection:
            return self._dictionary_row("entry", selection[0])
        selection = self._dictionary_candidates_tree.selection()
        if selection:
            return self._dictionary_row("candidate", selection[0])
        return None

    def _dictionary_row(self, kind: str, iid: str) -> CorrectionEntry | None:
        try:
            index = int(iid)
        except (TypeError, ValueError):
            return None
        rows = self._dictionary_entries if kind == "entry" else self._dictionary_candidates
    ...
    def _refresh_check(self) -> None:
        ...
        correction_path = self._correction_memory_path()
        rows.append(self._check_line(correction_path.exists(), "correction dictionary file", str(correction_path)))
        rows.append(self._check_line((_USER_DIR / "personal_dictionary.json").exists(), "personal dictionary file", str(_USER_DIR / "personal_dictionary.json")))
    ...
    def _open_dictionary_file(self) -> None:
        path = self._correction_memory_path()
        path.parent.mkdir(parents=True, exist_ok=True)
        if not path.exists():
            path.write_text('{"version": 1, "entries": [], "candidates": []}', encoding="utf-8")
        os.startfile(str(path))
    ...
    def _correction_memory_path(self) -> Path:
        cfg = _read_config()
        correction = cfg.get("correction_memory") or {}
        raw_path = str(correction.get("path") or "").strip()
        if raw_path:
            return Path(raw_path).expanduser()
        return _USER_DIR / "correction_memory.json"

    def _new_correction_memory(self) -> CorrectionMemory:
        cfg = _read_config()
        return CorrectionMemory.from_config(cfg.get("correction_memory") or {})

    @staticmethod
    def _toggle_set_value(values: set, value) -> None:
        if value in values:
            values.remove(value)
        else:
            values.add(value)
