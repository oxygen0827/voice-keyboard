...
class CorrectionMemory:
    def __init__(
        ...
        *,
        ...
        self._confirm_threshold = max(1, int(confirm_threshold or 3))
    ...
    def from_config(cls, cfg: dict | None) -> "CorrectionMemory":
        ...
        return cls(
            path=path,
            confirm_threshold=max(3, int(cfg.get("confirm_threshold", 3) or 3)),
...
class CorrectionLearningTracker:
    def __init__(
        ...
        *,
        ...
        read_clipboard_text: Callable[[str], str | CorrectionTextSnapshot | None] | None = None,
    ...
    def from_config(
        ...
        clipboard_probe_enabled = bool(cfg.get("clipboard_probe_fallback", False))
        return cls(
            memory,
            read_current_text,
            read_screen_text=read_screen_text if screen_ocr_enabled else None,
            read_clipboard_text=(
                read_clipboard_text if clipboard_probe_enabled else None
            ),
    ...
    def _observe_pending(
        ...
        before = pending.text
        snapshot = self._maybe_read_clipboard_snapshot(pending, snapshot)
        snapshot = self._maybe_read_screen_snapshot(pending, snapshot)
        observed, source = _choose_observed_text(pending, before, snapshot)
        snapshot_inferred = infer_correction_pairs(before, observed)
        committed_inferred = _committed_replacement_inferred(pending)
        inferred = _merge_inferred_corrections(
            snapshot_inferred,
            committed_inferred,
        )
        same_segment = _looks_like_same_segment_edit(before, observed)
        ...
        if (
            ...
            and not _has_repeated_inferred_evidence(snapshot_inferred)
            and not committed_inferred
        ):
            decision = CorrectionCaptureDecision(
                decision="skipped",
            ...
            return CorrectionObservationResult(decisions=(decision,))
        if (
            pending.keyboard_edit_seen
            and source == "tracked_segment"
            and observed != pending.shadow_text
            and not committed_inferred
        ):
            decision = CorrectionCaptureDecision(
                decision="skipped",
                reason="tracked_segment_replaced_pending",
                source=source,
                detail=snapshot.detail,
                before=before,
                after=observed,
        ...
        if (
            ...
            decision = CorrectionCaptureDecision(
                decision="skipped",
                reason="current_text_not_same_segment",
                source=source,
                detail=snapshot.detail,
                before=before,
                after=observed,
    ...
    def _maybe_read_clipboard_snapshot(
        self,
        pending: _PendingCorrectionObservation,
        snapshot: CorrectionTextSnapshot,
    ) -> CorrectionTextSnapshot:
        if self._read_clipboard_text is None:
            return snapshot
        if not _should_try_clipboard_probe(
            pending,
            snapshot,
            now=self._clock(),
            after_edit_seconds=self._screen_ocr_after_edit_seconds,
            ...
            return snapshot
        try:
            clipboard_snapshot = _coerce_snapshot(
                self._read_clipboard_text(pending.text)
            )
        except Exception as e:
            print(f"[correction] 剪贴板探测读取失败: {e}")
            return snapshot
        if not clipboard_snapshot.text:
            self._debug_log(
                "剪贴板探测无文本 "
                f"source={clipboard_snapshot.source} detail={clipboard_snapshot.detail!r}"
            )
            return snapshot
        if not _screen_ocr_snapshot_is_relevant(
            pending.text,
            clipboard_snapshot.text,
        ):
            self._debug_log(
                "剪贴板探测已跳过：与待学习片段重叠不足 "
                f"source={clipboard_snapshot.source} "
                f"text={_preview(clipboard_snapshot.text)!r}"
            )
            return snapshot
        return clipboard_snapshot
    ...
    def _record_new_evidence(
        ...
        for correction in inferred:
            ...
            if learned is None:
                continue
            pending.recorded_evidence[key] = max(previous, evidence_count)
            if learned.newly_confirmed:
                confirmed.append(learned)
            else:
                candidates.append(learned)
...
def _record_pending_deletion(
    ...
    context_start = max(0, start - (_MAX_TERM_LENGTH - 1))
    context_end = min(len(text), end + (_MAX_TERM_LENGTH - 1))
    ...
    *,
    ...
    for index in indexes:
        ...
        if char == replacement or not _CJK_RE.search(char):
            continue
        for wrong, correct in _candidate_terms(
            text,
            index,
            index + 1,
            replacement,
        ):
            if (wrong, correct) not in emitted:
...
def _has_repeated_inferred_evidence(
    inferred: tuple[InferredCorrection, ...],
    minimum: int = 2,
) -> bool:
    return any(item.evidence_count >= minimum for item in inferred)
...
def _should_try_clipboard_probe(
    pending: _PendingCorrectionObservation,
    snapshot: CorrectionTextSnapshot,
    *,
    now: float,
    ...
    if snapshot.source == "tracked_segment":
        return True
    if snapshot.source.startswith("shadow+tracked_segment"):
        return True
    return _looks_like_incomplete_ime_edit(pending.text, current)
...
def _replacement_changes(before: str, after: str) -> list[tuple[int, int, str]]:
    ...
    for tag, a0, a1, b0, b1 in matcher.get_opcodes():
        ...
        if tag == "delete":
            ...
            return []
        wrong = before[a0:a1]
        replacement_text = after[b0:b1]
        if a1 == len(before) and b1 == len(after) and len(replacement_text) > len(wrong):
            replacement_text = replacement_text[:len(wrong)]
        replacement = _replacement_term(replacement_text)
...
def _candidate_terms(
    ...
    token_start, token_end = _cjk_token_bounds(before, start, end)
    min_start = max(token_start, end - _MAX_TERM_LENGTH)
    max_start = min(start, token_end - _MIN_TERM_LENGTH)
    for term_start in range(min_start, max_start + 1):
        min_end = max(end, term_start + _MIN_TERM_LENGTH)
        max_end = min(token_end, term_start + _MAX_TERM_LENGTH)
        for term_end in range(min_end, max_end + 1):
            wrong = before[term_start:term_end]
            prefix = before[term_start:start]
            suffix = before[end:term_end]
...
def _cjk_token_bounds(text: str, start: int, end: int) -> tuple[int, int]:
    token_start = max(0, int(start))
    token_end = max(token_start, int(end))
    value = str(text or "")
    while token_start > 0 and _CJK_RE.match(value[token_start - 1]):
        token_start -= 1
    while token_end < len(value) and _CJK_RE.match(value[token_end]):
        token_end += 1
