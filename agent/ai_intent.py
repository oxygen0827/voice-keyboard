...
class ReusableTextMemoryEntries(Protocol):
...
class ReusableTextMemoryAliases(Protocol):
...
class IntentContext:
    ...
    shortcuts: tuple[str, ...] = ()
    reusable_text_memory_records: tuple[ReusableTextMemoryRecord, ...] = ()

    @property
    def reusable_text_memory_keys(self) -> tuple[str, ...]:
        return tuple(record.key for record in self.reusable_text_memory_records)
...
class IntentFallbackOptions:
    ...
    reusable_text_memory_fuzzy_recall: bool = True
...
- Instruction Mode，有以下几种 Voice Keyboard Operation：
  ...
  * 编辑：修改/润色/删除 Explicit Selection（明确选区），或在用户明确说“全文/全部/整个输入框”等整体范围时处理当前输入环境窗口
...
5. 用户要保存可复用文本片段（"记一下"、"记住"、"备忘"、"存一下"等关键词）。key 是用户给这条文本起的名字（如"手机号"、"邮箱"、"家庭地址"），value 是要保存的文本：
   ...
   → {"type":"reusable_text_save","key":"...","value":"..."}
6. 用户要查询已保存的可复用文本（"我的xxx是什么"、"我的xxx"、"xxx是多少"、"xxx是啥"等问句）。
   ...
   只要用户的问题在语义上对得上某个已保存的 key，就大胆返回 reusable_text_recall。key 字段必须填【已保存列表里那个原始 key】，不要返回用户口述的版本。
   → {"type":"reusable_text_recall","key":"已保存列表里的原始key"}
7. 用户要删除可复用文本（"忘记我的xxx"、"删掉xxx的记录"） → {"type":"reusable_text_delete","key":"..."}
8. 用户要查看/列出所有可复用文本（"列出所有备忘"、"我都记了什么"、"导出记忆库"、"看一下我的记忆"等） → {"type":"reusable_text_list"}
...
def apply_intent_fallbacks(
    ...
    if (
        ...
        return {"type": "edit"}
    if fallbacks.reusable_text_memory_fuzzy_recall and intent == "reusable_text_recall":
...
def reusable_text_memory_records(
    entries: ReusableTextMemoryEntries | None,
    lexicon: ReusableTextMemoryAliases | None = None,
) -> tuple[ReusableTextMemoryRecord, ...]:
    if entries is None:
        return ()
    records = []
    for key in entries.keys():
        aliases = lexicon.aliases_for(key) if lexicon is not None else ()
        records.append(ReusableTextMemoryRecord(
            key=key,
...
def _build_user_message(ctx: IntentContext) -> str:
    ...
    if ctx.reusable_text_memory_keys:
        user_msg += (
            "已保存的可复用文本名称："
            f"{'、'.join(ctx.reusable_text_memory_keys)}\n"
        )
    else:
