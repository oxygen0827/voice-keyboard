...
class WindowsTrayApp:
    ...
    def run(self) -> None:
        ...
        if self._start_enabled():
            self._enable_voice_keyboard(notify=False)
    ...
    def _menu(self):
        labels = self._labels()
        return pystray.Menu(
            pystray.MenuItem(labels["running"] if self._backend_enabled else labels["standby"], lambda: None, enabled=False),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem(labels["open_main_window"], self._open_main_window, default=True),
            pystray.MenuItem(
                labels["enable_voice_keyboard"] if not self._backend_enabled else labels["disable_voice_keyboard"],
                self._toggle_voice_keyboard,
            ),
            pystray.Menu.SEPARATOR,
    ...
    def _labels(self) -> dict[str, str]:
        if self._language() == "en":
            return {
                ...
                "standby": "Voice Keyboard is on standby",
                "open_main_window": "Open Main Window",
                "enable_voice_keyboard": "Enable Voice Keyboard",
                "disable_voice_keyboard": "Disable Voice Keyboard",
                "language": "Language",
                "language_zh": "Chinese",
                "language_en": "English",
                "hotkeys": "Hotkeys",
        ...
        return {
            ...
            "standby": "Voice Keyboard \u5b89\u5168\u5f85\u673a",
            "open_main_window": "\u6253\u5f00\u4e3b\u7a97\u53e3",
            "enable_voice_keyboard": "\u542f\u7528\u8bed\u97f3\u952e\u76d8",
            "disable_voice_keyboard": "\u505c\u7528\u8bed\u97f3\u952e\u76d8",
    ...
    def _message(self, key: str, **values) -> str:
        ...
        messages = {
            ...
            "voice_enabled": ("Voice Keyboard enabled", "\u8bed\u97f3\u952e\u76d8\u5df2\u542f\u7528"),
            "voice_disabled": ("Voice Keyboard disabled", "\u8bed\u97f3\u952e\u76d8\u5df2\u505c\u7528"),
            ...
            "insert_blocked_standby": (
                "Enable Voice Keyboard before inserting text",
                "\u8bf7\u5148\u542f\u7528\u8bed\u97f3\u952e\u76d8\uff0c\u518d\u63d2\u5165\u6587\u672c",
            ),
        }
    ...
    def _insert_text_after_menu_closes(self, text: str, kind: str) -> None:
        def _run():
            ...
            try:
                with self._lock:
                    env = (
                        self._backend.input_environment
                        if self._backend_enabled and self._backend is not None
                        else None
                    )
                if env is None:
                    self._notify("insert_blocked_standby")
                    return
                if env is not None:
    ...
    def _toggle_voice_keyboard(self, _icon=None, _item=None):
        if self._backend_enabled:
            self._disable_voice_keyboard()
        else:
            self._enable_voice_keyboard()

    def _enable_voice_keyboard(self, notify: bool = True):
        if self._backend_enabled:
            return
        ...
        if notify:
            self._notify("voice_enabled")
        self._refresh_menu()

    def _disable_voice_keyboard(self):
        if not self._backend_enabled:
            return
        self._backend_enabled = False
        self._stop_backend()
        self._status.set_state("idle")
        self._notify("voice_disabled")
        self._refresh_menu()
    ...
    def _start_enabled(self) -> bool:
        cfg = self._read_config()
        ui = cfg.get("ui") or {}
        return bool(ui.get("start_enabled", True))
