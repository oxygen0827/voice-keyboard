...
class _OpenAISTT:
    def __init__(self, cfg: dict):
        ...
        self._prompt   = str(cfg.get("prompt") or "").strip()
...
class _ZhipuSTT:
    ...
    def __init__(self, cfg: dict):
        ...
        self._language = cfg.get("language", "zh")
        self._prompt_hint = str(cfg.get("prompt") or "").strip()
    ...
    def _build_prompt(self) -> str:
        ...
        hints = []
        if self._prompt_hint:
            hints.append(self._prompt_hint)
        if self._hotwords:
            hints.append(
                "Hotwords / personal dictionary: "
                + "、".join(str(word) for word in self._hotwords)
            )
        if hints:
            prompt = prompt + "\n\n" + "\n".join(hints)
        return prompt
...
class _TypeUpBackendSTT:
    """
    通过 TypeUp 后端代理语音识别。

    所需配置：
      api_base_url       后端地址，默认 http://localhost:8000
      access_token       登录后获得的 access token
      refresh_token      可选，用于 access token 过期后刷新
      cloud_bridge_path  可选，TypeUp Desktop 写入的登录态 JSON
    """

    def __init__(self, cfg: dict):
        self._auth = TypeUpBackendAuth(cfg, log_prefix="stt")
    ...
    def transcribe(self, pcm: bytes) -> str:
        ...
        if resp.status_code == 401 and self._auth.refresh_token:
            ...
            resp = self._post_transcribe(wav)
        if not resp.ok:
            raise RuntimeError(self._auth.error_message(resp, "TypeUp 后端 STT 请求失败"))
        return (resp.json().get("text") or "").strip()

    def _post_transcribe(self, wav: bytes):
        return requests.post(
            f"{self._auth.api_base_url}/v1/stt/transcribe",
