...
Voice Keyboard Engine is the local voice input context. It turns a user's spoken intent into text or text operations in the current input environment, while account, subscription, payment, and cloud entitlement concepts belong outside this context.
...
A user-requested change or action applied to text in the current **Input Environment** based on speech.
...
**Instruction Mode**:
The mode where speech is treated as an instruction for a **Voice Text Operation**.
...
**Shortcut Invocation**:
A **Voice Text Operation** that triggers a named system or application shortcut.
...
- **Dictation** is one kind of **Voice Text Operation**.
- **Dictation Mode** produces **Dictation**.
- **Instruction Mode** interprets speech as a request for a **Voice Text Operation**.
...
- The project was described as "voice typing"; resolved: the broader intent is **Voice Text Operation**, with **Dictation** as one operation type.
