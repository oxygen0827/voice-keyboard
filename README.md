...
打完字之后，说错了或想改，不需要手动找位置、选中、重打——按住编辑键说出修改指令，松开自动完成：

| 场景 | 你说的话 | 结果 |
|------|---------|------|
| 改关键词 | "把会议改成会谈" | 原文中「会议」→「会谈」 |
| 删除片段 | "去掉最后一句" | 自动删除最后一句 |
| 追加内容 | "在后面加上请及时回复" | 句尾追加文字 |
| 改语气 | "改成更正式的表达" | LLM 润色重写 |

**工作原理：**
1. 按住 `edit_key`（默认右 Ctrl）说出修改指令，松开
2. 指令发给 STT 识别成文字
3. 原文 + 指令一起发给 LLM，拿回修改后的文字
4. Agent 发退格键擦掉原文，逐字打入新文字

需要在 `config.yaml` 中配置 `llm` 段（API Key）。LLM 和 STT 可以用同一家的 Key。
